...
static void InputNumberInAnotherSystem() // Получаем данные от пользователя
{
    ...
    int number1 = ChangeSystemOfNumberInTenSystem(result1, firstSystem1, endSystem);
    int number2 = ChangeSystemOfNumberInTenSystem(result2, firstSystem2, endSystem);
...
static int ChangeSystemOfNumberInTenSystem(string number,int firstSystem,int endSystem) // Перевод из одной системы счисления в другую числа пользователя
{
    bool negative = false;
    string testOfCharString = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
    string result = "";
